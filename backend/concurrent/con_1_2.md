...
由于在获取锁时，通常采用CAS方式进行竞争，为了减少在大多数情况下只有一个线程访问共享变量（竞争不激烈）的情况下获取锁的开销，提出使用偏向锁，由于偏向锁机制是**默认**开启的，且由于一般使用synchronized关键字时，不会采用自旋CAS的方式来修改共享变量，**所以**，在开启偏向锁时，进入synchronized代码块时一般为无锁（无状态）转化为偏向锁。
