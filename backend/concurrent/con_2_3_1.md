...
- 中断
  ...
  当线程调用park方法后被中断时，不会抛出`InterruptedException`异常

- 虚假唤醒

  调用park的线程同样存在虚假唤醒的问题，为了避免线程调用park后没有收到unpark就被唤醒，需要将park的调用放入循环条件判断中，如果判断条件不满足则继续调用park
...
这样，队列中所有为SHARED的点，最终状态都会变为PROPAGATE，由于最终会执行shouldParkAfterFailedAcquire方法，其中会将所有PROPAGATE转换为SIGNAL，也即最终队列中**只有SIGNAL状态，就一定会调用到unparkSuccessor**
