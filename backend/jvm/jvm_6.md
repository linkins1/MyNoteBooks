...
.java文件需要经过javac编译成class文件，之后再通过jvm去进行**加载、链接、初始化**。

.c文件的编译过程包含了**预处理-编译-汇编-链接**，得到机器码，之后执行**装入**将逻辑地址和物理地址进行映射后创建PCB在机器上运行。
...
![image-20200921154950686](https://cdn.jsdelivr.net/gh/linkins1/MyNoteBooks/resources/imgs/jvm/image-20200921154950686.png)
...
用描述符来描述方法时，按照**先参数列表、后返回值的顺序**描述，参数列表严格按照方法中参数的顺序放在一组小括号“()”之内。
