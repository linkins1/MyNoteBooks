...
>
> ***A：***在JDK1.7之前，JVM管理的全部内存（包括方法区）与本地内存都存在于RAM中，两者是独立的两部分，且永久代有最大内存限制；在JDK1.8后，JVM的内存框架中的方法区中由元空间实现的部分存放于本地内存中，且没有单独的最大值限制，这样做是为了在运行期间创建大量代理类时不会因为受到方法区内存空间的限制而频繁出现的OOM
...
为了保证每个线程可以独立正确的执行，需要给每个线程都分配一个**独立的程序计数器**来完成程序的流的正确标记。
...
> 常量池表用于存放编译期生成的各种字面量、类加载后的直接引用以及和类的版本、字段、方法、接口等描述信息
