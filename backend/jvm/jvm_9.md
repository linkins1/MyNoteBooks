...
这里单独说一下最后一步，也即字节码生成的步骤，`<init>()`和`<clinit>()`这两个构造器在这个阶段被添加，两者的区别如下
- `<init>()`
    - 会先调用父类的`<init>()`构造器
    - 其不等价于默认构造器，还包括了类中的普通代码块“{}”（用于给类中的实例变量初始化并赋值）
- `<clinit>()`
    - 不会调用父类的`<clinit>()`构造器，但可能会调用Object的`<init>()`构造器
    - 由编译器自动收集类中的所有类变量的赋值动作和静态语句块“static{}”中的语句合并产生的，编译器收集的顺序是由语句在源文件中出现的顺序决定的
    
但是两者的共同点是两者的产生实际上是一种代码收敛的过程，都是由编译器把语句块按顺序收集到这两个构造器中
