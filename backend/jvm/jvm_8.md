...
在方法被调用时，就会进行局部变量表中的参数值到参数列表传递的过程，也即实参到形参的传递(方法参数)与方法内部定义的局部变量的赋值。
...
从上面可以看出，进行动态分派时是要**唯一确定**一个实际类型，而静态分派时是根据参数的静态类型的不同可以**执行多个**重载的方法。总结下来为**动态单分派，静态多分派**，动态单分派指在转型时必须确定出唯一确定的类型，如上面的`son.hardChoice(new QQ())`，这时首先要动态的唯一确定son的实际类型为`Son`（单分派），之后，由于`Son`中存在两个重载方法，这时需要根据静态类型来进行选择（多分派），由于此例中静态类型为`QQ`，所以为执行`QQ`对应的重载方法（编译期确定）
